...
# procs
	proc Add {type flags cmd {procname ""}} {
		if {![regexp {^(pub2?|pubm|nick|mode|raw|client|bot|time|evnt|dcc|kick)$} $type]} {
	...
	proc ProcessPubm {nick user handle channel text} {
		...
		foreach n [binds pub2] {
			if { [string match [lindex $n 2] $text] } {
				[lindex $n 4] $nick $user $handle $channel $text
			}
		}
	...
	proc ProcessRaw { direction line } {
		if { [string equal $direction "server"] } {
			set direction "raw"
		}
		foreach n [binds $direction] {
			[lindex $n 4] $line
		}
	}
