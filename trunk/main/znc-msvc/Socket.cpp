...
int CZNCSock::GetAddrInfo(const CS_STRING &sHostname, CSSockAddr &csSockAddr) {
	...
	if(WSAStringToAddress(const_cast<char*>(sHostname.c_str()), AF_INET6, NULL, (sockaddr*)&address, &address_length) == 0) {
		sockaddr_in6 *paddr = (sockaddr_in6*)&address;
		*csSockAddr.GetAddr6() = paddr->sin6_addr;
#endif
	...
	if(WSAStringToAddress(const_cast<char*>(sHostname.c_str()), AF_INET, NULL, (sockaddr*)&address, &address_length) == 0) {
		sockaddr_in *paddr = (sockaddr_in*)&address;
		*csSockAddr.GetAddr() = paddr->sin_addr;
#endif
